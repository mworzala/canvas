...
import com.mattworzala.canvas.compiler.v2.CanvasErrors.FRAGMENT_FUN_MAIN
import com.mattworzala.canvas.compiler.v2.CanvasErrors.FRAGMENT_PROPERTY_BACKING_FIELD
import com.mattworzala.canvas.compiler.v2.CanvasErrors.FRAGMENT_SUSPEND_FUN
import com.mattworzala.canvas.compiler.v2.CanvasErrors.FRAGMENT_VAR
...
class ComposableDeclarationChecker : DeclarationChecker, StorageComponentContainerContributor {
    ...
    private fun checkFunction(
        ...
        if (descriptor.overriddenDescriptors.isNotEmpty()) {
            ...
            if (override.hasComposableAnnotation() != hasComposableAnnotation) {
                context.trace.report(
                    CanvasErrors.CONFLICTING_OVERLOADS.on(
                        declaration,
        ...
        if (descriptor.isSuspend && hasComposableAnnotation) {
            context.trace.report(
                FRAGMENT_SUSPEND_FUN.on(declaration.nameIdentifier ?: declaration)
            )
        }
        ...
        if (hasComposableAnnotation &&
            ...
            context.trace.report(
                FRAGMENT_FUN_MAIN.on(declaration.nameIdentifier ?: declaration)
            )
        }
    }
    ...
    private fun checkType(
        ...
        if (type.hasComposableAnnotation() && type.isSuspendFunctionType) {
            context.trace.report(
                FRAGMENT_SUSPEND_FUN.on(element)
            )
        }
    }
    ...
    private fun checkProperty(
        ...
        if (initializer != null && name != null) {
            context.trace.report(FRAGMENT_PROPERTY_BACKING_FIELD.on(name))
        }
        if (descriptor.isVar && name != null) {
            context.trace.report(FRAGMENT_VAR.on(name))
        }
    }
